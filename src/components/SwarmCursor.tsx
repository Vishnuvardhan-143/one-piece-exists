...
const SwarmCursor: React.FC<SwarmCursorProps> = ({
  ...
  useEffect(() => {
    ...
    const frame = (now: number) => {
      ...
      compProgram.uniforms.uTime.value = t;
    ...
    // ==========================================
    // FALLING GLITTER SPARKS CANVAS OVERLAY
    // ==========================================
    const sparkCanvas = document.createElement('canvas');
    sparkCanvas.className = 'swarm-cursor__sparkles-canvas';
    sparkCanvas.style.position = 'absolute';
    sparkCanvas.style.inset = '0';
    sparkCanvas.style.width = '100%';
    sparkCanvas.style.height = '100%';
    sparkCanvas.style.pointerEvents = 'none';
    sparkCanvas.style.zIndex = '2';
    container.appendChild(sparkCanvas);

    const ctx = sparkCanvas.getContext('2d');
    let sparkWidth = container.clientWidth || 1;
    let sparkHeight = container.clientHeight || 1;

    const resizeSpark = () => {
      sparkWidth = container.clientWidth || 1;
      sparkHeight = container.clientHeight || 1;
    ...
    resizeSpark();
    const sparkRo = new ResizeObserver(resizeSpark);
    sparkRo.observe(container);
    ...
    const sparks: Spark[] = [];
    let lastSparkPos = { x: cssW * 0.5, y: cssH * 0.5 };
    const sparkColors = ['#FFD700', '#5FFBF1', '#FFFFFF', '#00FFFF', '#FFF8DC'];

    const spawnSparksAt = (x: number, y: number, amount: number) => {
      for (let k = 0; k < amount; k++) {
        ...
        sparks.push({
          ...
          vx: Math.cos(angle) * speed * 0.5 + (Math.random() - 0.5) * 20,
          vy: Math.sin(angle) * speed * 0.3 + 10 + Math.random() * 35, // Initial downward drift
          size: 1.5 + Math.random() * 3.5,
          life: 0,
          maxLife: 0.6 + Math.random() * 0.8, // 0.6s to 1.4s life
    ...
    const renderSparks = (now: number) => {
      sparkRaf = requestAnimationFrame(renderSparks);
      if (!ctx) return;

      const dt = Math.min((now - lastSparkTime) / 1000, 0.05);
      lastSparkTime = now;

      // Spawn falling sparks when cursor moves
      if (cursor.has) {
        const dist = Math.hypot(cursor.x - lastSparkPos.x, cursor.y - lastSparkPos.y);
        if (dist > 6) {
          const spawnCount = Math.min(6, Math.floor(dist / 8));
          spawnSparksAt(cursor.x, cursor.y, spawnCount);
          lastSparkPos = { x: cursor.x, y: cursor.y };
        }
      }

      ctx.clearRect(0, 0, sparkWidth, sparkHeight);

      const gravity = 120; // Falling gravity acceleration
      ...
      for (let i = sparks.length - 1; i >= 0; i--) {
        ...
        // Apply physics: downward gravity + air drag
        s.vy += gravity * dt;
        s.vx *= 0.96;
        s.x += s.vx * dt;
        s.y += s.vy * dt;
        ...
        const fadeOut = Math.max(0, 1 - Math.pow(progress, 2));
        const alpha = fadeIn * fadeOut;

        ctx.save();
        ctx.translate(s.x, s.y);
        ctx.rotate(s.rotation);
        ctx.globalAlpha = alpha * propsRef.current.opacity;

        // Draw glittering shiny star/sparkle shape
    ...
    return () => {
      ...
      if (sparkCanvas.parentElement === container) container.removeChild(sparkCanvas);
